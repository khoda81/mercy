...
fn decode_choices(bytes: &[u8], probabilities: &[u16]) -> Vec<u8> {
    let mut decoder = RangeDecoder::new(bytes);
    let mut choices = Vec::with_capacity(probabilities.len());

    for &raw in probabilities {
        choices.push(u8::from(decoder.test(FractionalU16::from_raw(raw))));
    }
...
fn decode_encode<O: Emission>(source: &[u8], probabilities: &[u16]) -> (usize, u64) {
    let choices = decode_choices(source, probabilities);
    let bytes = encode::<O>(probabilities, &choices);
    let checksum = bytes.iter().fold(0u64, |checksum, &byte| {
        checksum.rotate_left(1) ^ byte as u64
    });
    (bytes.len(), checksum)
}

fn decode_encode_production(source: &[u8], probabilities: &[u16]) -> (usize, u64) {
    let choices = decode_choices(source, probabilities);
    let bytes = encode_production(probabilities, &choices);
    let checksum = bytes.iter().fold(0u64, |checksum, &byte| {
        checksum.rotate_left(1) ^ byte as u64
    });
    (bytes.len(), checksum)
...
fn workload(events: usize) -> (Vec<u8>, Vec<u16>) {
    ...
    // Two random source bits per event is comfortably above the expected
    // entropy of Bernoulli probabilities uniformly distributed over [0.5, 1).
    let source = (0..events.div_ceil(4))
        .map(|_| random() as u8)
        .collect();
...
fn coder_round_trip(c: &mut Criterion) {
    ...
    let (source, probabilities) = workload(EVENTS);
    let choices = decode_choices(&source, &probabilities);
    ...
    let canonical_choices = decode_choices(&production_bytes, &probabilities);
    assert_eq!(canonical_choices, choices);

    let recanonicalized = encode_production(&probabilities, &canonical_choices);
    assert_eq!(recanonicalized, production_bytes);

    eprintln!(
        "canonicalization workload: {EVENTS} events, {} source bytes -> {} canonical bytes ({:.4} bits/event)",
        source.len(),
    ...
    group.bench_function("isize", |b| {
        b.iter(|| {
            black_box(decode_encode::<IsizeOutput>(
                black_box(&source),
    ...
    group.bench_function("tagged", |b| {
        b.iter(|| {
            black_box(decode_encode::<TaggedOutput>(
                black_box(&source),
                black_box(&probabilities),
            ))
        });
    ...
    group.bench_function("production", |b| {
        b.iter(|| {
            black_box(decode_encode_production(
                black_box(&source),
                black_box(&probabilities),
            ))
        });
