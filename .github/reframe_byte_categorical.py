...
path = Path("src/serde_model.rs")
text = path.read_text()

start = text.index("/// A non-negative, not-necessarily-normalized prediction over byte-sized choices.")
end = text.index("/// Stateful predictor for the canonical Mercy representation of `T`.")
replacement = r'''/// The exact byte distribution consumed by Mercy's entropy coder.
///
...
text = text.replace("type Prediction<'a>: Prediction", "type Prediction<'a>: IntoByteCategorical")

start = text.index("/// A normalized categorical distribution over one [`Choices`] domain.")
end = text.index("#[derive(Debug)]", start)
replacement = r'''/// Entropy-encoder seam. A real range/arithmetic coder implements this trait.
...
old = '''        let prediction = self.model.predict();
...
new = '''        let distribution = {
        ...
        self.model.observe(choice);'''
if old not in text:
    raise SystemExit("serializer choose pattern not found")
...
new = '''        let distribution = {
        ...
        self.model.observe(choice);'''
if old not in text:
    raise SystemExit("deserializer choose pattern not found")
text = text.replace(old, new, 1)

old = r'''    impl Prediction for HistoryPrediction {
...
new = r'''    impl IntoByteCategorical for HistoryPrediction {
        fn byte_categorical(&self) -> ByteCategorical {
...
if old not in text:
    raise SystemExit("HistoryPrediction pattern not found")
text = text.replace(old, new, 1)
...
if old not in text:
    raise SystemExit("TraceStep pattern not found")
text = text.replace(old, new, 1)

old = r'''    impl ChoiceEncoder for TraceEncoder {
...
new = r'''    impl ChoiceEncoder for TraceEncoder {
        fn encode(&mut self, choice: u8, distribution: &ByteCategorical) -> Result<()> {
            self.steps.push(TraceStep {
                choice,
...
if old not in text:
    raise SystemExit("TraceEncoder pattern not found")
text = text.replace(old, new, 1)

old = r'''    impl ChoiceDecoder for TraceDecoder<'_> {
...
new = r'''    impl ChoiceDecoder for TraceDecoder<'_> {
        fn decode(&mut self, distribution: &ByteCategorical) -> Result<u8> {
            let step = &self.steps[self.cursor];
            self.cursor += 1;
...
if old not in text:
    raise SystemExit("TraceDecoder pattern not found")
text = text.replace(old, new, 1)
